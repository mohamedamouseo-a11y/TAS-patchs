...
BASE_URL="https://raw.githubusercontent.com/mohamedamouseo-a11y/TAS-patchs/main/TAS-SERVICE-CAPACITY-PHASE-8-PREMIUM-BOOKING-FLOW-V1"
...
TMP="$(mktemp -d /tmp/tas-service-phase8.XXXXXX)"
PAYLOAD="$TMP/payload"
...
rollback_live() {
  ...
  if [ "$MIGRATION_ATTEMPTED" = "1" ] && [ -f "$CURRENT/scripts/rollback-tas-premium-booking-v1.ts" ]; then
    (
      cd "$CURRENT"
      pnpm exec tsx scripts/rollback-tas-premium-booking-v1.ts
    ) >/dev/null 2>&1 || true
  fi
...
# Phase 7 + Developer Hub snapshot fix must be present in live/canonical baseline.
for root in "$CURRENT" "$REPO"; do
  grep -q 'selectTASAutoBayCandidateForLoad' "$root/server/tasDb.ts" || { echo "DEPLOY=FAIL"; echo "ERROR=PHASE7_ASSIGNMENT_NOT_PRESENT"; exit 3; }
  grep -q 'Bay / الكوريك' "$root/client/src/pages/tas/TASServicePage.tsx" || { echo "DEPLOY=FAIL"; echo "ERROR=PHASE7_UI_NOT_PRESENT"; exit 3; }
...
curl -fsSL "$BASE_URL/payload/client/src/components/tas/TASPremiumBookingFlow.tsx" -o "$PAYLOAD/client/src/components/tas/TASPremiumBookingFlow.tsx"
curl -fsSL "$BASE_URL/payload/scripts/apply-tas-premium-booking-v1.ts" -o "$PAYLOAD/scripts/apply-tas-premium-booking-v1.ts"
curl -fsSL "$BASE_URL/payload/scripts/verify-tas-premium-booking-v1.ts" -o "$PAYLOAD/scripts/verify-tas-premium-booking-v1.ts"
curl -fsSL "$BASE_URL/payload/scripts/rollback-tas-premium-booking-v1.ts" -o "$PAYLOAD/scripts/rollback-tas-premium-booking-v1.ts"
curl -fsSL "$BASE_URL/payload/snippets/tasDb-availability-v3.ts.txt" -o "$PAYLOAD/snippets/tasDb-availability-v3.ts.txt"
curl -fsSL "$BASE_URL/payload/snippets/tasDb-premium-booking.ts.txt" -o "$PAYLOAD/snippets/tasDb-premium-booking.ts.txt"
...
for rel in "${NEW_TARGETS[@]}"; do
  [ ! -e "$CURRENT/$rel" ] && [ ! -L "$CURRENT/$rel" ] || { echo "DEPLOY=FAIL"; echo "ERROR=PHASE8_ALREADY_PRESENT_ACTIVE_$rel"; exit 3; }
  [ ! -e "$REPO/$rel" ] && [ ! -L "$REPO/$rel" ] || { echo "DEPLOY=FAIL"; echo "ERROR=PHASE8_ALREADY_PRESENT_CANONICAL_$rel"; exit 3; }
done
...
if [ -n "$CANON_STATUS_BEFORE" ]; then
  printf '%s\n' "$CANON_STATUS_BEFORE"
  echo "DEPLOY=FAIL"
  echo "ERROR=CANONICAL_PHASE8_TARGETS_DIRTY"
  exit 3
fi
...
git -C "$PATCH_TREE" config user.email "phase8@tas.local"
git -C "$PATCH_TREE" config user.name "TAS Phase 8"
...
git -C "$PATCH_TREE" diff --binary --no-ext-diff HEAD -- . > "$TMP/phase8.patch"

[ -s "$TMP/phase8.patch" ] || { echo "DEPLOY=FAIL"; echo "ERROR=EMPTY_PHASE8_PATCH"; exit 4; }
grep -q '^diff --git a/' "$TMP/phase8.patch" || { echo "DEPLOY=FAIL"; echo "ERROR=PATCH_NOT_GIT_STYLE"; exit 4; }

(
  cd "$CURRENT"
  patch --batch --forward --fuzz=0 --dry-run -p1 < "$TMP/phase8.patch" >/dev/null
)
...
(
  cd "$CURRENT"
  patch --batch --forward --fuzz=0 -p1 < "$TMP/phase8.patch" >/dev/null
)
ACTIVE_MUTATED=1

grep -q 'plannedDurationMinutes: int("plannedDurationMinutes")' "$CURRENT/shared/schema.ts" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_SCHEMA_PATCH_VERIFY_FAILED"; exit 5; }
grep -q 'bookingMode === "premium_v1"' "$CURRENT/server/tasDb.ts" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_PREMIUM_BACKEND_VERIFY_FAILED"; exit 5; }
grep -q 'maintenanceIntervalId: Number(input?.maintenanceIntervalId' "$CURRENT/server/routers.ts" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_ROUTER_VERIFY_FAILED"; exit 5; }
grep -q 'TASPremiumBookingFlow' "$CURRENT/client/src/pages/tas/TASServicePage.tsx" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_PREMIUM_UI_VERIFY_FAILED"; exit 5; }
...
(
  cd "$CURRENT"
  pnpm exec tsx scripts/apply-tas-premium-booking-v1.ts --apply
  pnpm exec tsx scripts/verify-tas-premium-booking-v1.ts
)
echo "MIGRATION=PASS"
...
grep -q 'plannedDurationMinutes: int("plannedDurationMinutes")' "$REPO/shared/schema.ts" || { echo "SOURCE_SYNC=FAIL"; echo "ERROR=CANONICAL_SCHEMA_SYNC_FAILED"; exit 8; }
grep -q 'bookingMode === "premium_v1"' "$REPO/server/tasDb.ts" || { echo "SOURCE_SYNC=FAIL"; echo "ERROR=CANONICAL_BACKEND_SYNC_FAILED"; exit 8; }
grep -q 'TASPremiumBookingFlow' "$REPO/client/src/pages/tas/TASServicePage.tsx" || { echo "SOURCE_SYNC=FAIL"; echo "ERROR=CANONICAL_UI_SYNC_FAILED"; exit 8; }

CANON_MUTATED=0

echo "PM2=$PM2_STATUS"
echo "HTTP=$HTTP"
echo "PREMIUM_BOOKING_FLOW=ACTIVE"
echo "PREMIUM_SLOT_ENFORCEMENT=ACTIVE"
echo "MAINTENANCE_DURATION_OVERRIDE=ACTIVE"
echo "AUTO_BAY_ASSIGNMENT=PRESERVED"
echo "GENERAL_SERVICE_MODE=PRESERVED"
echo "ARBITRARY_TIME_INPUT=REMOVED_FROM_PREMIUM_FLOW"
