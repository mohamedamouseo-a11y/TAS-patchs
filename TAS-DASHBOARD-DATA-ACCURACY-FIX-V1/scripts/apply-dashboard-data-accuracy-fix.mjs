...
function read(rel) {
  return fs.readFileSync(path.join(targetRoot, rel), 'utf8');
}

function write(rel, content) {
  fs.writeFileSync(path.join(targetRoot, rel), content, 'utf8');
}

function replaceExact(rel, oldText, newText) {
  const current = read(rel);
  if (!current.includes(oldText)) {
    throw new Error(`Expected source block not found in ${rel}`);
  }
  if (current.indexOf(oldText) !== current.lastIndexOf(oldText)) {
    throw new Error(`Expected source block is not unique in ${rel}`);
  }
