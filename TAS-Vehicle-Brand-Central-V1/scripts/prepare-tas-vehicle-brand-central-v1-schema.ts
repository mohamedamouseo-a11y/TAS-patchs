...
const q = async (sql: string, values: any[] = []) => connection.query(sql, values);
const scalar = async (sql: string, values: any[] = []) => Number(((await q(sql, values))[0] as any[])[0]?.n ?? 0);

async function tableExists(table: string) {
  return (await scalar(`SELECT COUNT(*) n FROM information_schema.tables WHERE table_schema=? AND table_name=?`, [databaseName, table])) === 1;
}
async function columnExists(table: string, column: string) {
  return (await scalar(`SELECT COUNT(*) n FROM information_schema.columns WHERE table_schema=? AND table_name=? AND column_name=?`, [databaseName, table, column])) === 1;
}
async function indexExists(table: string, indexName: string) {
  return (await scalar(`SELECT COUNT(*) n FROM information_schema.statistics WHERE table_schema=? AND table_name=? AND index_name=?`, [databaseName, table, indexName])) > 0;
}
