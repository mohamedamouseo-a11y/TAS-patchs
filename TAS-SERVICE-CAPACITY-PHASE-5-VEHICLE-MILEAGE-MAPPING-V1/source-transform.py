...
def copy_payload(rel: str) -> None:
    ...
    if dst.exists():
        ...
        if dst.read_bytes() == incoming:
            return
        raise RuntimeError("conflicting existing Phase 5 payload target: " + rel)
    dst.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(src, dst)

schema_rel = "shared/schema.ts"
schema = read_file(root, schema_rel)
if "export const tasMaintenanceVehicleMappings = mysqlTable(" not in schema:
...
if "listTASMaintenanceVehicleMappings," not in router:
    anchor = "  updateTASMaintenanceInterval,\n"
    snippet = read_file(payload, "snippets/router-import.txt")
    router = replace_once(router, anchor, anchor + snippet, "router maintenance import")

if "listMaintenanceVehicleMappings: tasPermissionProcedure" not in router:
    anchor = "    updateMaintenanceInterval: tasPermissionProcedure.input(tasAnyInput).mutation(async ({ input }) => updateTASMaintenanceInterval(Number(input?.id ?? input?.intervalId), input ?? {})),\n"
    snippet = read_file(payload, "snippets/router-endpoints.txt")
    count = router.count(anchor)
    if count != 2:
        raise RuntimeError("service router maintenance interval anchor expected twice, found " + str(count))
...
if "TASMaintenanceVehicleMappingSettings" not in page:
...
print("PHASE5_SOURCE_TRANSFORM=PASS")
