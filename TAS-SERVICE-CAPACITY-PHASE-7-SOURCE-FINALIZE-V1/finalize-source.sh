...
grep -q 'selectTASAutoBayCandidateForLoad' "$ACTIVE/server/tasDb.ts" || { echo "SOURCE_FINALIZE=FAIL"; echo "ERROR=LIVE_PHASE7_ASSIGNMENT_MISSING"; exit 3; }
grep -q 'FOR UPDATE' "$ACTIVE/server/tasDb.ts" || { echo "SOURCE_FINALIZE=FAIL"; echo "ERROR=LIVE_PHASE7_LOCKING_MISSING"; exit 3; }
grep -q 'Bay / الكوريك' "$ACTIVE/client/src/pages/tas/TASServicePage.tsx" || { echo "SOURCE_FINALIZE=FAIL"; echo "ERROR=LIVE_PHASE7_UI_MISSING"; exit 3; }
[ -f "$ACTIVE/scripts/verify-tas-auto-bay-assignment-v1.ts" ] || { echo "SOURCE_FINALIZE=FAIL"; echo "ERROR=LIVE_PHASE7_VERIFY_FILE_MISSING"; exit 3; }
...
TMP="$(mktemp -d /tmp/tas-phase7-source-finalize.XXXXXX)"
BASE="$TMP/base"
MERGED="$TMP/merged"
BACKUP="$TMP/backup"
...
grep -q 'selectTASAutoBayCandidateForLoad' "$MERGED/server/tasDb.ts" || { echo "SOURCE_FINALIZE=FAIL"; echo "ERROR=MERGED_ASSIGNMENT_MARKER_MISSING"; exit 4; }
grep -q 'Bay / الكوريك' "$MERGED/client/src/pages/tas/TASServicePage.tsx" || { echo "SOURCE_FINALIZE=FAIL"; echo "ERROR=MERGED_UI_MARKER_MISSING"; exit 4; }
...
grep -q 'selectTASAutoBayCandidateForLoad' "$REPO/server/tasDb.ts" || { echo "SOURCE_FINALIZE=FAIL"; echo "ERROR=SOURCE_ASSIGNMENT_VERIFY_FAILED"; exit 5; }
grep -q 'Bay / الكوريك' "$REPO/client/src/pages/tas/TASServicePage.tsx" || { echo "SOURCE_FINALIZE=FAIL"; echo "ERROR=SOURCE_UI_VERIFY_FAILED"; exit 5; }
...
echo "LIVE_PHASE7_CONFIRMED=YES"
