...
curl -fsSL "$BASE_URL/source-transform-v2.py" -o "$TMP/source-transform.py"
...
for required in   "shared/schema.ts"   "server/tasDb.ts"   "server/routers.ts"   "client/src/pages/tas/TASServicePage.tsx"; do
...
sed   -e "s|$TMP/active-before/|a/|g"   -e "s|$TMP/active-after/|b/|g"   "$TMP/raw.abs.patch" > "$TMP/phase3.patch"
