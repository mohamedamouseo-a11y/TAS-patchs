...
BASE_URL="https://raw.githubusercontent.com/mohamedamouseo-a11y/TAS-patchs/main/TAS-SERVICE-CAPACITY-PHASE-11-MAINTENANCE-ITEMS-COSTS-V1"
...
TMP="$(mktemp -d /tmp/tas-service-phase11.XXXXXX)"
PAYLOAD="$TMP/payload"
...
rollback_live() {
  ...
  if [ "$MIGRATION_ATTEMPTED" = "1" ] && [ -f "$CURRENT/scripts/rollback-tas-maintenance-items-costs-v1.ts" ]; then
    (
      cd "$CURRENT"
      pnpm exec tsx scripts/rollback-tas-maintenance-items-costs-v1.ts
    ) >/dev/null 2>&1 || true
  fi
...
curl -fsSL "$BASE_URL/payload/client/src/components/tas/TASMaintenanceItemsCostsSettings.tsx" -o "$PAYLOAD/client/src/components/tas/TASMaintenanceItemsCostsSettings.tsx"
curl -fsSL "$BASE_URL/payload/scripts/apply-tas-maintenance-items-costs-v1.ts" -o "$PAYLOAD/scripts/apply-tas-maintenance-items-costs-v1.ts"
curl -fsSL "$BASE_URL/payload/scripts/verify-tas-maintenance-items-costs-v1.ts" -o "$PAYLOAD/scripts/verify-tas-maintenance-items-costs-v1.ts"
curl -fsSL "$BASE_URL/payload/scripts/rollback-tas-maintenance-items-costs-v1.ts" -o "$PAYLOAD/scripts/rollback-tas-maintenance-items-costs-v1.ts"
curl -fsSL "$BASE_URL/payload/snippets/tasDb-maintenance-items-costs-v1.ts.txt" -o "$PAYLOAD/snippets/tasDb-maintenance-items-costs-v1.ts.txt"
...
for rel in "${NEW_TARGETS[@]}"; do
  [ ! -e "$CURRENT/$rel" ] && [ ! -L "$CURRENT/$rel" ] || { echo "DEPLOY=FAIL"; echo "ERROR=PHASE11_ALREADY_PRESENT_ACTIVE_$rel"; exit 3; }
  [ ! -e "$REPO/$rel" ] && [ ! -L "$REPO/$rel" ] || { echo "DEPLOY=FAIL"; echo "ERROR=PHASE11_ALREADY_PRESENT_CANONICAL_$rel"; exit 3; }
done
...
git -C "$PATCH_TREE" config user.email "phase11@tas.local"
git -C "$PATCH_TREE" config user.name "TAS Phase 11"
...
git -C "$PATCH_TREE" diff --binary --no-ext-diff HEAD -- . > "$TMP/phase11.patch"

[ -s "$TMP/phase11.patch" ] || { echo "DEPLOY=FAIL"; echo "ERROR=EMPTY_PHASE11_PATCH"; exit 4; }
grep -q '^diff --git a/' "$TMP/phase11.patch" || { echo "DEPLOY=FAIL"; echo "ERROR=PATCH_NOT_GIT_STYLE"; exit 4; }

(
  cd "$CURRENT"
  patch --batch --forward --fuzz=0 --dry-run -p1 < "$TMP/phase11.patch" >/dev/null
)
...
(
  cd "$CURRENT"
  patch --batch --forward --fuzz=0 -p1 < "$TMP/phase11.patch" >/dev/null
)
ACTIVE_MUTATED=1

grep -q 'tasMaintenanceItems' "$CURRENT/shared/schema.ts" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_ITEM_SCHEMA_MISSING"; exit 5; }
grep -q 'getTASMaintenanceIntervalPackage' "$CURRENT/server/tasDb.ts" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_PACKAGE_BACKEND_MISSING"; exit 5; }
grep -q 'snapshotTASMaintenancePackageForBooking' "$CURRENT/server/tasDb.ts" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_BOOKING_SNAPSHOT_MISSING"; exit 5; }
grep -q 'getMaintenanceIntervalPackage' "$CURRENT/server/routers.ts" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_PACKAGE_ROUTE_MISSING"; exit 5; }
grep -q 'TASMaintenanceItemsCostsSettings' "$CURRENT/client/src/pages/tas/TASServicePage.tsx" || { echo "DEPLOY=FAIL"; echo "ERROR=ACTIVE_PHASE11_UI_MISSING"; exit 5; }
...
(
  cd "$CURRENT"
  pnpm exec tsx scripts/apply-tas-maintenance-items-costs-v1.ts --apply
  pnpm exec tsx scripts/verify-tas-maintenance-items-costs-v1.ts
)
echo "MIGRATION=PASS"
...
echo "PM2=$PM2_STATUS"
echo "HTTP=$HTTP"
echo "MAINTENANCE_ITEM_CATALOG=ACTIVE"
echo "INTERVAL_PACKAGE_LINES=ACTIVE"
echo "BOOKING_ITEM_SNAPSHOT=ACTIVE"
echo "PACKAGE_TOTALS=ACTIVE"
echo "PREPARATION_FLAG=ACTIVE"
echo "SOURCE_DATA_SEED=NOT_INCLUDED_BY_DESIGN"
echo "PHASE7_AUTO_BAY=PRESERVED"
echo "PHASE8_PREMIUM_BOOKING=PRESERVED"
echo "PHASE9_SCHEDULER=PRESERVED"
echo "PHASE10_LIFECYCLE=PRESERVED"
