...
async function main() {
  ...
  if (!fs.existsSync(STATE)) {
    console.log("TAS_PARTS_PREPARATION_ROLLBACK=PASS");
  ...
  try {
    ...
    if (state.table === "created") {
      await db.query("DROP TABLE IF EXISTS tas_service_booking_item_preparation");
    }
...
main().catch((error) => {
  console.error("TAS_PARTS_PREPARATION_ROLLBACK=FAIL");
