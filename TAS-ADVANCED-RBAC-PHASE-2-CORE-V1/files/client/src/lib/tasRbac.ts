...
export type TasFeaturePermissionMatrix = Record<string, TasFeatureGrant | undefined>;
...
const FEATURE_MODULE_FALLBACK: Record<string, TasRbacModule> = {
  "dashboard.home": "dashboard",
  "conversations.inbox": "conversations",
  "sales.overview": "sales",
  "sales.leads": "sales",
  "sales.lead_profile": "sales",
  "sales.excel_imports": "sales",
  "sales.competitive_queues": "sales",
  "sales.quotations": "sales",
  "sales.tasks": "sales",
  "sales.test_drives": "sales",
  "sales.trade_ins": "sales",
  "sales.finance_applications": "sales",
  "catalog.vehicles": "catalog",
  "catalog.inventory": "catalog",
  "catalog.brands": "catalog",
  "finance.overview": "finance",
  "service.overview": "service",
  "after_sales.overview": "after_sales",
  "operations.overview": "operations",
  "reports.overview": "reports",
  "marketing.overview": "marketing",
  "shipping.agent": "shipping",
  "integrations.whatsapp_cloud": "integrations",
  "integrations.whatsapp_gateway": "integrations",
  "admin.overview": "admin",
  "admin.users": "users",
  "admin.roles_permissions": "roles",
  "admin.audit_log": "audit_log",
  "admin.system_settings": "system_settings",
};
...
export function useTasRbac(enabled = true) {
  ...
  const featurePermissions = ((query.data as any)?.featurePermissions ?? {}) as TasFeaturePermissionMatrix;

  const canModule = (module: TasRbacModule, action: TasRbacAction = "view") =>
    canFromTasMatrix(permissions, module, action);
  const can = canModule;
  const featureGrant = (featureKey: string): TasFeatureGrant | null => featurePermissions[featureKey] ?? null;
  const canFeature = (featureKey: string, action: TasRbacAction = "view"): boolean => {
    const grant = featureGrant(featureKey);
    if (grant) return Boolean(grant[action]);
    const fallbackModule = FEATURE_MODULE_FALLBACK[featureKey];
    return fallbackModule ? canModule(fallbackModule, action) : false;
  };
