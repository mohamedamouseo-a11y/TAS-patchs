...
export default function TASPermissionGuard({
  ...
  const allowed = feature ? rbac.canFeature(feature, action) : rbac.canModule(module, action);

  if (rbac.error || (adminOnly && !isAdmin) || !allowed) {
    const permissionLabel = feature ? `${feature}.${action}` : `${module}.${action}`;
    return (
      <CRMLayout>
