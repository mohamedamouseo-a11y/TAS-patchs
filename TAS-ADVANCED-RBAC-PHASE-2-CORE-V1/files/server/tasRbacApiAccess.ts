...
import { getTasRbacFeature, inferTasRbacFeature, type TasRbacFeatureKey } from "./tasRbacFeatureCatalog";
...
export type TasApiAccessDecision = {
  ...
  featureKey: TasRbacFeatureKey | null;
...
export function inferTasRbacModule(path: string): TasRbacModule | null {
  ...
  // AUD-002: Explicit tas.sales.* namespace before keyword matching
  if (normalized.startsWith("tas.sales.")) return "sales";
...
async function featureOverride(roleKey: string, featureKey: TasRbacFeatureKey): Promise<Partial<Record<TasRbacAction, boolean>> | null> {
  if (roleKey === "Admin") return null;
  const [rows] = await getPool().query<any[]>(
    `SELECT p.can_view, p.can_create, p.can_edit, p.can_delete, p.can_export, p.can_approve, p.can_assign
     FROM tas_rbac_feature_permissions p
     INNER JOIN tas_rbac_roles r ON r.id=p.role_id
     WHERE r.role_key=? AND r.is_active=1 AND p.feature_key=?
     LIMIT 1`,
    [roleKey, featureKey],
  );
  const row = rows[0];
...
export async function authorizeTasApiRequest(
  ...
  const featureKey = inferTasRbacFeature(path);
  if (featureKey) {
    const feature = getTasRbacFeature(featureKey);
    if (feature?.module === module && requestedRole !== "Admin") {
      const override = await featureOverride(requestedRole, featureKey);
      if (override && !override[action]) {
        throw new TRPCError({ code: "FORBIDDEN", message: `Missing permission: ${featureKey}.${action}` });
      }
      if (override && action !== "view" && !override.view) {
        throw new TRPCError({ code: "FORBIDDEN", message: `Missing permission: ${featureKey}.view` });
      }
    }
  }
