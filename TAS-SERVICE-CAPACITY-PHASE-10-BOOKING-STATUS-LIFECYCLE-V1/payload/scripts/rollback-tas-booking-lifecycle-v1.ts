...
async function main() {
  ...
  if (!fs.existsSync(STATE)) {
    console.log("TAS_BOOKING_LIFECYCLE_ROLLBACK=PASS");
  ...
  try {
    ...
    if (state.tableCreated) {
      await db.query("DROP TABLE IF EXISTS tas_service_booking_status_history");
    }
    console.log("TAS_BOOKING_LIFECYCLE_ROLLBACK=PASS");
...
main().catch((error) => {
  console.error("TAS_BOOKING_LIFECYCLE_ROLLBACK=FAIL");
